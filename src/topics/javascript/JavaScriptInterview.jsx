...
import { javascriptInterviewPart1 } from "../../data/javascript/javascriptInterviewPart1";
import { javascriptInterviewPart2 } from "../../data/javascript/javascriptInterviewPart2";

// Split into two files to keep each file small and manageable.
// Part 1 = Questions 1-50, Part 2 = Questions 51-100.
const interviewData = javascriptInterviewPart1 + javascriptInterviewPart2;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmedLine.startsWith("```")) {
      if (!inCodeBlock) {
        inCodeBlock = true;
        codeBlockLanguage =
          trimmedLine.replace(/```/g, "").trim() || "javascript";
        codeBlockContent = [];
      } else {
        inCodeBlock = false;
...
function JavaScriptInterview() {
  ...
  return (
    <div className="js-interview-page">
      {/* Hero Section */}
      <section className="interview-header">
        <div className="interview-header-content">
          <Link to="/javascript" className="back-button">
            ← Back to JavaScript
          </Link>
          <h1 className="interview-title">JavaScript Interview Questions</h1>
          <p className="interview-subtitle">
            Master JavaScript interview questions with detailed answers and code
            examples
          </p>
                                ...
                                if (item.type === "code") {
                                  return (
                                    <div key={idx} className="code-block">
                                      ...
                                      <pre
                                        className={`language-${item.language || "javascript"}`}
                                      >
                                        <code
                                          className={`language-${item.language || "javascript"}`}
